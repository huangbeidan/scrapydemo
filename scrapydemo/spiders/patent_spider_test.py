...
id_dict = defaultdict(lambda: "null")


class InfoboxSpider(scrapy.Spider):
    name = 'patent_spider_test'
    ...
    def connect(self):
        ...
        try:
            ...
            # execute a statement
            # cur.execute(
            #     'select distinct paperid, affiliation_colab from google_scholar_authornetwork where colabs_country is null;')

            cur.execute(
                'select idx, affiliation_colab from bd_patent2 where mark is null and mark2 is null;')
    ...
    def _clean(self, name):
        ...
        unwanted = ["Member_of_", "Professor_of_", "Assoc_Professor_at_", "Visiting_Professor_", "Professor_in_",
                    "Director_of_ ", "Professor_for_", "Associate_Professor_with_Tenure_",
                    "Research_Associate_Professor_", "Research_Director_", "Professor_", "Associate_Professor_",
                    "Professor_and_Head_", "Director_", "PhD_student_", "Professor_retired_", "Research_scientist_",
                    "Asst_Professor_of_", "Adjunct_Professor_of_", "Sr_Technical_Officer_in_", "Senior_Scientific_Researcher_",
                    "Assistant_professor_", "Lecturer_in_", "Assistant_Professor_of_", "Research_fellow_",
                    "CEO_", "Research_Assistant_Professor_", "Chief_Technology_Officer_", "Post_Doctoral_Fellow_",
                    "Senior_Scientist_"]
    ...
    def parse(self, response):
        ...
        if len(info_card) > 2 and (info_card["Location"] != "null" or info_card["Headquarters"] != "null"):
            yield {
